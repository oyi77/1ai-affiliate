...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if (! $error) {
		...
		if ($editing == true) {
			...
			$aff_campaign_sql = "INSERT INTO `aff_campaigns` SET";
		}
...
if (isset($_GET['delete_aff_campaign_id'])) {
	...
	if (!hash_equals((string) ($_SESSION['token'] ?? ''), (string) ($_GET['token'] ?? ''))) {
		header('location: ' . get_absolute_url() . 'tracking_support/setup/aff_campaigns.php');
		die();
	}
					...
					<?php if ($editing == true) { ?>
						<div class="row">
							...
							<div class="col-xs-6">
								...
								<button type="submit" class="btn btn-sm btn-danger btn-block" onclick="window.location='<?php echo get_absolute_url(); ?>tracking_support/setup/aff_campaigns.php'; return false;">Cancel</button>
							</div>
						</div>
					<?php } else { ?>
...
<script type="text/javascript">
	$(document).ready(function() {
		<?php if ($getDlDniRow && isset($getDlDniRow['id'])) {
			...
			tablesorterPagerOptions.ajaxUrl = "<?php echo get_absolute_url(); ?>tracking_support/ajax/dni_get_offers.php?all_offers&dni=" + dni + "&offset=0&limit=25&column&filter[0]=<?php echo $dlOfferId; ?>";
