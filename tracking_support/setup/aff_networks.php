...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if (! $error) {
		...
		if ($editing == true) {
			$aff_network_sql = "UPDATE `aff_networks` SET";
		} else {
			$aff_network_sql = "INSERT INTO `aff_networks` SET";
		}
...
if (isset($_GET['delete_aff_network_id'])) {
	...
	if (!hash_equals((string) ($_SESSION['token'] ?? ''), (string) ($_GET['token'] ?? ''))) {
		header('location: ' . get_absolute_url() . 'tracking_support/setup/aff_networks.php');
		die();
	}
					...
					<ul class="list">
						...
						while ($aff_network_row = $aff_network_result->fetch_array(MYSQLI_ASSOC)) {
							...
							if (!empty($aff_network_row['dni_network_id']))
								$html['network_logo'] = '<img src="/img/favicon.gif" width=16>&nbsp;&nbsp;';
