...
# replace excelfile.xls with whatever you want the filename to default to
	header("Content-Disposition: attachment; filename=Tvisitors_".time().".xls");
...
LEFT JOIN text_ads ON (text_ads.text_ad_id = 2c.text_ad_id) 
...
//this is alittle different
		if ($click_row['click_cloaking']) {
			...
			if (!$click_row['click_alp']) { 
				$html['cloaking'] = htmlentities( 'http://' .$_SERVER['SERVER_NAME'] . get_absolute_url().'tracking_support/redirect/cl.php?pci=' . $click_row['click_id_public'] );
				$html['cloaking_host'] = htmlentities( (string) $_SERVER['SERVER_NAME'] );   
			} else { 
				//advanced lander
				$html['cloaking'] = htmlentities( 'http://' .$_SERVER['SERVER_NAME'] . get_absolute_url().'tracking_support/redirect/off.php?acip='. $click_row['aff_campaign_id_public'] . '&pci=' . $click_row['click_id_public'] );
				$html['cloaking_host'] = htmlentities( (string) $_SERVER['SERVER_NAME'] );   
			}
		} else {
			$html['cloaking'] = '';
