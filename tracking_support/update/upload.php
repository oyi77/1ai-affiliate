...
switch ($case) {
	...
	case 1:
		...
		about_revenue_upload();
			echo '<div class="row">
					<div class="col-xs-12">
						<form enctype="application/x-www-form-urlencoded" action="'.get_absolute_url().'tracking_support/update/upload.php" method="get">';
	...
	case 2:
		...
		if ( (!is_numeric($_GET['click_id'])) or (!is_numeric($_GET['click_payout'])) ) {
			...
			template_top('Upload Revenue Reports');
			about_revenue_upload();
			echo '<div class="error"><small><span class="fui-alert"></span>You forgot to check the subid and the commission column, <a href="'.get_absolute_url().'tracking_support/update/upload.php?case=1&file='.rawurlencode($name).'">please try again</a></small></div>';
			template_bottom();
			die();
	...
	default:
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST') { 
			...
			if (!$error) { 
				...
				header('location: '.get_absolute_url().'tracking_support/update/upload.php?case=1&file='.$file); die();
			}
		}
		...
		<div class="row">
			<div class="col-xs-12">
				<form enctype="multipart/form-data" action="<?php echo get_absolute_url();?>tracking_support/update/upload.php" method="post" class="form-horizontal" role="form">
