	...
	while ($count < ($_POST['counter']+1)) {
		...
		if ($_POST['offer_type'.$count] == 'campaign') {
			...
			if ($aff_campaign_id != 0) {
				...
				$outbound_go = '//' . getTrackingDomain() . get_absolute_url(). 'tracking_support/redirect/go.php?acip=' . $aff_campaign_row['aff_campaign_id_public'];
...
?>';
			...
			if ($rotator_id != 0) {
				...
				//for each real campaign selected, display the code to be used for it
				$outbound_go = '//' . getTrackingDomain() . get_absolute_url().'tracking_support/redirect/go.php?rpi=' . $rotator_row['public_id'];
