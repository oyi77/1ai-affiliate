...
$sql = "UPDATE  clicks LEFT JOIN clicks_advance USING (click_id) 
						   LEFT JOIN clicks_site USING (click_id) 
						   LEFT JOIN aff_campaigns ON (clicks.aff_campaign_id = aff_campaigns.aff_campaign_id)
						   LEFT JOIN aff_networks ON (aff_campaigns.aff_network_id = aff_networks.aff_network_id)
						   LEFT JOIN ppc_accounts ON (ppc_accounts.ppc_account_id = clicks.ppc_account_id)
						   LEFT JOIN ppc_networks ON (ppc_networks.ppc_network_id = ppc_accounts.ppc_network_id)
			SET     click_cpc='" . $mysql['click_cpc'] . "'
			WHERE   clicks.user_id='" . $mysql['user_id'] . "'";
if (isset($mysql['aff_network_id']) && $mysql['aff_network_id']) {
	$sql .= " AND aff_networks.aff_network_id='" . $mysql['aff_network_id'] . "' ";
}
if (isset($mysql['aff_campaign_id']) && $mysql['aff_campaign_id']) {
	$sql .= " AND clicks.aff_campaign_id='" . $mysql['aff_campaign_id'] . "' ";
}
if (isset($mysql['text_ad_id']) && $mysql['text_ad_id']) {
	$sql .= " AND clicks_advance.text_ad_id='" . $mysql['text_ad_id'] . "' ";
}
if (isset($mysql['landing_page_id']) && $mysql['landing_page_id']) {
	$sql .= " AND clicks.landing_page_id='" . $mysql['landing_page_id'] . "' ";
}
if (isset($mysql['ppc_network_id']) && $mysql['ppc_network_id']) {
	$sql .= " AND ppc_networks.ppc_network_id='" . $mysql['ppc_network_id'] . "' ";
}
if (isset($mysql['ppc_account_id']) && $mysql['ppc_account_id']) {
	$sql .= " AND clicks.ppc_account_id='" . $mysql['ppc_account_id'] . "' ";
}
