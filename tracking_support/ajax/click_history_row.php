...
if (!$click_row['type_name']) {
	$html['device_type'] = '<span id="device-tooltip"><span data-toggle="tooltip" title="Browser: ' . $html['browser_name'] . '<br/> Platform: ' . $html['platform_name'] . ' <br/>Device: ' . $html['device_name'] . '"><img title="' . $html['type_name'] . '" src="' . get_absolute_url() . 'img/icons/platforms/other.png"/></span></span>';
} else {
	$html['device_type'] = '<span id="device-tooltip"><span data-toggle="tooltip" title="Browser: ' . $html['browser_name'] . '<br/> Platform: ' . $html['platform_name'] . ' <br/>Device: ' . $html['device_name'] . '"><img title="' . $html['type_name'] . '" src="' . get_absolute_url() . 'img/icons/platforms/' . urlencode((string)$click_row['type_name']) . '.png"/></span></span> <img src="' . get_absolute_url() . 'img/icons/browsers/' . urlencode(getBrowserIcon($html['browser_name'])) . '.png">';
}
...
?>
					<tr <?php echo $tr_attrs ?? ''; ?>>
						...
						<td class="geo"><span data-toggle="tooltip" <?php echo 'title="' . $html['country_name'] . ' (' . $html['country_code'] . '), ' . $html['city_name'] . ' (' . $html['region_name'] . ')"'; ?>><img src="<?php echo get_absolute_url(); ?>img/flags/<?php echo strtolower((string) $html['country_code']); ?>.png"></span></td>
						...
						<td class="referer">
							<?php if ($html['referer'] != '') {
								printf('<a href="%s" target="_new" ><img src="%simg/icons/16x16/control_end_blue.png" alt="Referer" title="Referer: %s"/></a>', $html['referer'], get_absolute_url(), $html['referer']);
							} ?>
							<?php if ($html['landing'] != '') {
								printf('<a href="%s" target="_new"><img src="%simg/icons/16x16/control_pause_blue.png" alt="Landing"  title="Landing Page: %s"/></a>', $html['landing'], get_absolute_url(), $html['landing']);
							} ?>
							<?php if (($html['outbound'] != '') and ($click_row['click_out'] == 1)) {
								printf('<a href="%s" target="_new"><img src="%simg/icons/16x16/control_play_blue.png" alt="Outbound" title="Outbound: %s"/></a>', $html['outbound'], get_absolute_url(), $html['outbound']);
							} ?>
							<?php if (($html['cloaking'] != '') and ($click_row['click_out'] == 1)) {
								printf('<a href="%s" target="_new"><img src="%simg/icons/16x16/control_equalizer_blue.png" alt="Cloaking" title="Cloaked Referer: %s"/></a>', $html['cloaking'], get_absolute_url(), $html['cloaking']);
							} ?>
							<?php if (($html['redirect'] != '') and ($click_row['click_out'] == 1)) {
								printf('<a href="%s" target="_new"><img src="%simg/icons/16x16/control_fastforward_blue.png" alt="Redirection" title="Redirect: %s"/></a>', $html['redirect'], get_absolute_url(), $html['redirect']);
							} ?>
						</td>
						<td class="keyword">
