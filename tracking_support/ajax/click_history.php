...
LEFT JOIN clicks_site AS 2cs ON (2c.click_id = 2cs.click_id)
LEFT JOIN aff_campaigns AS 2ac ON (2c.aff_campaign_id = 2ac.aff_campaign_id)
LEFT JOIN ppc_accounts AS 2pa ON (2c.ppc_account_id = 2pa.ppc_account_id)
LEFT JOIN ppc_networks AS 2pn ON (2pa.ppc_network_id = 2pn.ppc_network_id)
LEFT JOIN landing_pages ON (landing_pages.landing_page_id = 2c.landing_page_id)
LEFT JOIN text_ads AS 2ta ON (2c.text_ad_id = 2ta.text_ad_id)
LEFT JOIN ips AS 2i ON (2c.ip_id = 2i.ip_id)
LEFT JOIN keywords AS 2k ON (2c.keyword_id = 2k.keyword_id)
LEFT JOIN browsers AS 2b ON (2c.browser_id = 2b.browser_id)
LEFT JOIN platforms AS 2p ON (2c.platform_id = 2p.platform_id)
LEFT JOIN device_models AS 2d ON (2c.device_id = 2d.device_id)
LEFT JOIN device_types ON (device_types.type_id = 2d.device_type)
LEFT JOIN locations_country AS 2cy ON (2c.country_id = 2cy.country_id)
LEFT JOIN locations_region AS 2rg ON (2c.region_id = 2rg.region_id)
LEFT JOIN locations_city ON (locations_city.city_id = 2c.city_id)
LEFT JOIN locations_isp AS 2is ON (2c.isp_id = 2is.isp_id)
LEFT JOIN site_urls AS 2su ON (2c.click_referer_site_url_id = 2su.site_url_id)
LEFT JOIN site_urls as 2cl ON (click_landing_site_url_id = 2cl.site_url_id)
LEFT JOIN site_urls as 2co ON (click_outbound_site_url_id = 2co.site_url_id)
LEFT JOIN site_urls as 2cc ON (click_cloaking_site_url_id = 2cc.site_url_id)
LEFT JOIN site_urls as 2credir ON (click_redirect_site_url_id = 2credir.site_url_id)
LEFT JOIN site_domains AS 2sd ON (2su.site_domain_id = 2sd.site_domain_id)
LEFT JOIN site_domains as 2cld ON (2cld.site_domain_id = 2cl.site_domain_id)
LEFT JOIN site_domains as 2cod ON (2cod.site_domain_id = 2co.site_domain_id)
LEFT JOIN site_domains as 2ccd ON (2ccd.site_domain_id = 2cc.site_domain_id)
LEFT JOIN site_domains as 2credird ON (2credird.site_domain_id = 2credir.site_domain_id)
";
...
<div class="row" style="margin-top: 10px;">
	...
	<div class="col-xs-6 text-right" style="top: -10px;">
		...
		<a style="font-size:12px;" target="_new" href="<?php echo get_absolute_url(); ?>tracking_support/visitors/download/">
			...
			<div class="pagination" id="table-pages">
				<ul>
					<?php if ($query['offset'] > 0) {
						printf(' <li class="previous"><a class="fui-arrow-left" onclick="loadContent(\'%stracking_support/ajax/click_history.php\',\'%s\',\'%s\');"></a></li>', get_absolute_url(), $query['offset'] - 1, $html['order'] ?? '');
					}
					...
					for ($i = 0; $i < $query['pages']; $i++) {
						if (($i >= $query['offset'] - 10) and ($i < $query['offset'] + 11)) {
							...
							printf(' <li %s><a onclick="loadContent(\'%stracking_support/ajax/click_history.php\',\'%s\',\'%s\');">%s</a></li>', $class, get_absolute_url(), $i, $html['order'] ?? '', $i + 1);
						}
					}

					if ($query['offset'] < $query['pages'] - 1) {
						printf(' <li class="next"><a class="fui-arrow-right" onclick="loadContent(\'%stracking_support/ajax/click_history.php\',\'%s\',\'%s\');"></a></li>', get_absolute_url(), $query['offset'] + 1, $html['order'] ?? '');
					}
					?>
				</ul>
			</div>
