...
//show tracking code
	...
	if (isset($_POST['edit_tracker']) && $_POST['edit_tracker'] && isset($_POST['tracker_id']) && $_POST['tracker_id']) {
		...
		$get_tracker_sql = "SELECT 
							...
							LEFT JOIN aff_networks USING (aff_network_id)
							LEFT JOIN text_ads USING (text_ad_id)
							LEFT JOIN ppc_accounts USING (ppc_account_id)
							LEFT JOIN ppc_networks USING (ppc_network_id)
							LEFT JOIN landing_pages ON (trackers.landing_page_id = landing_pages.landing_page_id) 
	...
	$tracker_sql = "INSERT INTO `trackers`
