...
$mysql['click_id'] = $conn->escape($_COOKIE['tracking1aisubid']);
$mysql['rpi'] = $conn->escape((string)$_GET['rpi']);
...
$rotator_sql = "SELECT
					   ...
					   lp.landing_page_url 	
				FROM rotators AS rt
				LEFT JOIN aff_campaigns AS ac ON ac.aff_campaign_id = rt.default_campaign
				LEFT JOIN landing_pages AS lp ON lp.landing_page_id = rt.default_lp
				LEFT JOIN users_pref AS up ON up.user_id = rt.user_id
				WHERE   rt.public_id='".$mysql['rpi']."'"; 
$rotator_row = memcache_mysql_fetch_assoc($db, $rotator_sql);
if (!$rotator_row) die();

$mysql['rotator_id'] = $conn->escape((string)$rotator_row['id']);
...
if ($default == false) {
		...
		$rule_redirects_sql = "SELECT
					   ...
					   lp.landing_page_url
				FROM clicks AS 2c
				LEFT JOIN clicks_record AS 2cr ON 2cr.click_id = 2c.click_id
				LEFT JOIN clicks_site AS 2cs ON 2cs.click_id = 2c.click_id	   
				LEFT JOIN rotator_rules_redirects AS rur ON rur.rule_id = '".$mysql['rule_id']."'
		...
		$rule_redirect_row = memcache_mysql_fetch_assoc($db, $rule_redirects_sql);
			...
			if (is_array($rule_redirect_row) && isset($rule_redirect_row['rule_redirect_id'])) {
				$mysql['rule_redirect_id'] = $conn->escape((string)$rule_redirect_row['rule_redirect_id']);
				$update_sql = "
					UPDATE
						clicks_rotator
					SET
			...
			if ($rule_redirect_row['redirect_campaign'] != null) {
				$mysql['aff_campaign_id'] = $conn->escape((string)$rule_redirect_row['aff_campaign_id']);
				$mysql['click_payout'] = $conn->escape((string)$rule_redirect_row['aff_campaign_payout']);

				$update_sql = "
					UPDATE
						clicks AS 2c
						LEFT JOIN clicks_spy AS 2cs ON (2c.click_id = 2cs.click_id)
					SET
				...
				<body>
					<form name="form1" id="form1" method="get"
						action="/tracking_support/redirect/cl2.php">
		...
		if ($rotator_row['default_campaign'] != null) {
			...
			$click_row = memcache_mysql_fetch_assoc($db, $click_sql);
				...
				$mysql['aff_campaign_id'] = $conn->escape((string)$rotator_row['aff_campaign_id']);
				$mysql['click_payout'] = $conn->escape((string)$rotator_row['aff_campaign_payout']);
