...
$change_pcustomer_api_key = false;
...
$personalSettingsPost = ['add_rest_api_key', 'remove_rest_api_key', 'update_account_currency', 'update_clickserver_api_key', 'change_user_api_key', 'change_user_statsapp_key', 'update_pcustomer_api_key'];
$personalSettingsGet = ['customers_api_key', 'remove_user_statsapp_key', 'remove_user_api_key'];
...
if ($userObj->hasPermission("access_to_personal_settings")) {
	...
	if ($html['user_statsapp_key']) $html['user_statsapp_key'] = $hiddenPart . substr($html['user_statsapp_key'], $hideChars, 99);
...
if (!empty($_POST['change_user_statsapp_key']) && $_POST['change_user_statsapp_key'] == '1') {
	if (!preg_match('/\*/', (string) $_POST['user_statsapp_key'])) {
		// Replace the undefined method with a more direct validation approach
		$app_key = $_POST['user_statsapp_key'];
		...
		if (!$error) {
			...
			//set the  session's user_api_key
			$_SESSION['user_statsapp_key'] = $_POST['user_statsapp_key'];
		}
	}
}

if (!empty($_POST['update_pcustomer_api_key']) && $_POST['update_pcustomer_api_key'] == '1') {
	if (!hash_equals((string)($_SESSION['token'] ?? ''), (string)($_POST['token'] ?? ''))) {
		$error['token'] = 'You must use our forms to submit data.';
	}
...
if (!empty($_POST['change_user_pass']) && $_POST['change_user_pass'] == '1') {
	...
	if (!isset($_POST['user_pass']) || empty($_POST['user_pass'])) {
		...
		$verify_stmt = $db->prepare('SELECT user_pass FROM users WHERE user_id = ? LIMIT 1');
		if ($verify_stmt) {
	...
	$update_stmt = $db->prepare('UPDATE users SET user_pass = ? WHERE user_id = ?');
	if ($update_stmt) {
		$current_user_id = (int) ($_SESSION['user_own_id'] ?? 0);
				...
				<?php if ($error) { ?>
					<div class="error" style="text-align:right"><small><span class="fui-alert"></span> <?php
																										echo ($error['token'] ?? '') .
																											...
																											($error['user_api_key'] ?? '') .
																											($error['user_pass'] ?? '') .
																											($error['pcustomer_api_key_invalid'] ?? '');
																										?></small></div>
				<?php } ?>
				<?php if ($change_pcustomer_api_key) { ?>
...
<?php if ($userObj->hasPermission("access_to_personal_settings")) { ?>
	...
	<div class="row account">
		...
		<div class="col-xs-8">
			<form class="form-horizontal" style="padding-top:0px;" role="form" method="post" action="">
				<input type="hidden" name="update_pcustomer_api_key" value="1" />
				<input type="hidden" name="token" value="<?php echo $_SESSION['token']; ?>" />
				<div class="form-group">
					<label for="pcustomer_api_key" class="col-xs-4 control-label">API key:</label>
					<div class="col-xs-8">
						<input type="text" class="form-control input-sm" id="pcustomer_api_key" name="pcustomer_api_key" value="<?php echo $html['pcustomer_api_key']; ?>">
					</div>
				</div>
				<div class="form-group">
					<div class="col-xs-8 col-xs-offset-4">
