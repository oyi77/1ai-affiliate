...
include_once(str_repeat("../", 1) . 'config/functions-icons.php');
include_once(str_repeat("../", 1) . 'config/functions-api.php');
include_once(str_repeat("../", 1) . 'config/functions-utils.php');
include_once(str_repeat("../", 1) . 'config/functions-empty.php');
...
$click_sql = "SELECT `AUTO_INCREMENT`-1 as clicks FROM  INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'clicks_counter';";
$click_row = memcache_mysql_fetch_assoc($click_sql);
$clicks = $click_row['clicks'];
//$click_sql = "SELECT count(*) clicks FROM clicks_total";
//$click_row = memcache_mysql_fetch_assoc($click_sql);
//$clicks += $click_row['clicks'];
...
if (isset($_POST['database_management'])) {
	...
	$tables = explode(',', 'clicks_advance,clicks_record,clicks_site,clicks_spy,clicks_tracking,clicks');
	...
	header('location: ' . get_absolute_url() . 'account/administration.php');
}
					...
					<div class="panel-body">
						...
						<p>
							Memcache Running <span class="fui-info-circle" style="font-size: 10px;"
								data-toggle="tooltip"
								title="If memcache is installed, but not running, check your config.php to make sure your connecting to a server that has memcache installed."></span><span
