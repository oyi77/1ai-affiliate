...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if (!$error) {
		...
		if ($editing === true) {
			...
			$user_sql = "INSERT INTO `users` SET";
		}
...
$user_result_delete = _mysqli_query($user_sql_delete);
	...
	$attributionCleanupQueries = [
		"DELETE FROM attribution_touchpoints WHERE snapshot_id IN (SELECT snapshot_id FROM attribution_snapshots WHERE user_id = " . $mysql['user_id'] . ")",
		"DELETE FROM attribution_snapshots WHERE user_id = " . $mysql['user_id'],
		"DELETE FROM attribution_settings WHERE user_id = " . $mysql['user_id'],
		"DELETE FROM attribution_models WHERE user_id = " . $mysql['user_id'],
		"DELETE FROM attribution_audit WHERE user_id = " . $mysql['user_id']
	];
					...
					<div class="form-group" style="margin-bottom: 0px;">
						...
						<div class="col-xs-5">
							<select class="form-control input-sm" name="user_role">
								<option value="2" <?php if (isset($html['role_id']) && $html['role_id'] == '2') echo "selected"; ?>>Admin</option>
								<option value="3" <?php if (isset($html['role_id']) && $html['role_id'] == '3') echo "selected"; ?>>Campaign manager</option>
								<option value="4" <?php if (isset($html['role_id']) && $html['role_id'] == '4') echo "selected"; ?>>Campaign optimizer</option>
								<option value="5" <?php if (isset($html['role_id']) && $html['role_id'] == '5') echo "selected"; ?>>Campaign viewer</option>
							...
							<?php if ($editing == true) { ?>
								<div class="row">
									...
									<div class="col-xs-6">
										...
										<button type="submit" class="btn btn-sm btn-danger btn-block" onclick="window.location='<?php echo get_absolute_url(); ?>account/user-management.php'; return false;">Cancel</button>
									</div>
								</div>
							<?php } else { ?>
