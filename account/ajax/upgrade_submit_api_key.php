<?php
declare(strict_types=1);
include_once(str_repeat("../", 2).'config/connect.php');
$conn = \OneAIAffiliate\Repository\LookupRepositoryFactory::connection($db);

AUTH::require_user(); 

if (isset($_POST['api_key'])) {
	if ($_POST['token'] != $_SESSION['token']) { $error['token'] = 'You must use our forms to submit data.';  }
	$mysql['pcustomer_api_key'] = $conn->escape((string)$_POST['api_key']);
	$mysql['user_id'] = $conn->escape((string)$_SESSION['user_own_id']);
	$validate = validateCustomersApiKey($mysql['pcustomer_api_key']);
	if ($validate['code'] != 200) {
		$error['pcustomer_api_key_invalid'] = "API key is not valid. Check your key and try again!";
	}
	if (!$error) {
		$conn->query("UPDATE users SET pcustomer_api_key = '".$mysql['pcustomer_api_key']."' WHERE user_id = '".$mysql['user_id']."'");
		$msg = ['error' => false, 'msg' => 'Valid'];
	} else {
		$msg = ['error' => true, 'msg' => $error['token'] . $error['pcustomer_api_key_invalid']];
	}

	echo json_encode($msg);
}

if (isset($_POST['get_alert_body'])) { ?>
	<small><p><?php echo $_SESSION['premium_pdetails']['body'];?></p></small>
	<small><p>Release date: <?php echo $_SESSION['premium_pdetails']['release-date'];?> - <a href="#changelogs" id="see_changelogs" data-toggle="modal" data-target="#changelogsPremium" style="color:#428bca; font-weight:normal; margin-top: 15px;">See what's new</a></p></small>
	<a style="margin-right:5px;" href="<?php echo get_absolute_url();?>account/auto-upgrade-premium.php" class="btn btn-xs btn-warning"><?php echo $_SESSION['premium_pdetails']['order-button-text'];?> ($<?php echo $_SESSION['premium_pdetails']['upgrade-price'];?>)</a>
<?php } ?>