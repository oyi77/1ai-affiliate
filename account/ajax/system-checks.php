...
function buildAttributionCronCheck(?\mysqli $connection): array
{
    ...
    $scriptPath = realpath(__DIR__ . '/../../cronjobs/attribution-rebuild.php');
    if ($scriptPath === false) {
        return [
            'id' => 'attribution_cron',
            'label' => $label,
            'status' => 'error',
            'details' => 'Cron script `cronjobs/attribution-rebuild.php` is missing. Restore the file before scheduling runs.',
        ];
    }

    $result = $connection->query('SELECT last_execution_time FROM cronjob_logs WHERE id = 2 LIMIT 1');
...
function buildAttributionExportCronCheck(?\mysqli $connection): array
{
    ...
    $scriptPath = realpath(__DIR__ . '/../../cronjobs/attribution-export.php');
    if ($scriptPath === false) {
        return [
            'id' => 'attribution_export_cron',
            'label' => $label,
            'status' => 'error',
            'details' => 'Cron script `cronjobs/attribution-export.php` is missing. Restore the file before scheduling runs.',
        ];
    }

    $result = $connection->query('SELECT last_execution_time FROM cronjob_logs WHERE id = 3 LIMIT 1');
    if (!$result || $result->num_rows === 0) {
        return [
            'id' => 'attribution_export_cron',
            'label' => $label,
