...
final class MysqlUserRepository implements UserRepositoryInterface
{
    ...
    public function list(int $offset, int $limit): array
    {
        $countStmt = $this->conn->prepareRead(
            'SELECT COUNT(*) AS total FROM users WHERE user_deleted = 0'
        );
        ...
        $stmt = $this->conn->prepareRead(
            'SELECT user_id, user_fname, user_lname, user_name, user_email, user_timezone,
             ...
             FROM users WHERE user_deleted = 0 ORDER BY user_id ASC LIMIT ? OFFSET ?'
        );
    ...
    public function create(array $data): int
    {
        ...
        return $this->conn->transaction(function () use ($data, $hashedPass, $now): int {
            ...
            $stmt = $this->conn->prepareWrite(
                'INSERT INTO users_pref (user_id) VALUES (?)'
            );
    ...
    {
        $stmt = $this->conn->prepareWrite(
            'UPDATE users SET user_deleted = 1 WHERE user_id = ?'
        );
        $this->conn->bind($stmt, 'i', [$id]);
        $this->conn->execute($stmt);
        $stmt->close();
    ...
    {
        $stmt = $this->conn->prepareRead(
            'SELECT user_id, api_key, created_at, scope FROM api_keys WHERE user_id = ?'
        );
        $this->conn->bind($stmt, 'i', [$userId]);
    ...
    {
        $stmt = $this->conn->prepareRead(
            'SELECT * FROM users_pref WHERE user_id = ? LIMIT 1'
        );
        $this->conn->bind($stmt, 'i', [$userId]);
    ...
    public function updatePreferences(int $userId, array $data): void
    {
        ...
        $stmt = $this->conn->prepareWrite(
            'UPDATE users_pref SET ' . implode(', ', $sets) . ' WHERE user_id = ?'
        );
        $this->conn->bind($stmt, $types, $values);
        $this->conn->execute($stmt);
        $stmt->close();
