...
)
{
    ...
    $count_sql = "SELECT count(*) AS count FROM dataengine AS 2c ";
    ...
    if ($pref_adv == true) {
        ...
        if ($user_row['user_pref_ppc_network_id'] and ! ($user_row['user_pref_ppc_account_id'])) {
            ...
            if (! preg_match('/ppc_accounts/', (string) $command)) {
                $command .= " LEFT JOIN ppc_accounts AS 2pa ON (2c.ppc_account_id = 2pa.ppc_account_id) ";
            }

            if (! preg_match('/ppc_networks/', (string) $command)) {
                $command .= " LEFT JOIN ppc_networks AS 2pn ON (2pa.ppc_network_id = 2pn.ppc_network_id) ";
            }
        }
        ...
        if ($user_row['user_pref_aff_network_id'] and ! ($user_row['user_pref_aff_campaign_id'])) {
            ...
            if (! preg_match('/aff_campaigns/', (string) $command)) {
                $command .= " LEFT JOIN aff_campaigns AS 2ac ON (2c.aff_campaign_id = 2ac.aff_campaign_id) ";
            }

            if (! preg_match('/aff_networks/', (string) $command)) {
                $command .= " LEFT JOIN aff_networks AS 2an ON (2ac.aff_network_id = 2an.aff_network_id) ";
            }
        }
        ...
        if ($user_row['user_pref_referer']) {
            ...
            if (! preg_match('/clicks_site/', (string) $command)) {
                $command .= " LEFT JOIN clicks_site AS 2cs ON (2c.click_id = 2cs.click_id) ";
            }

            if (! preg_match('/site_urls/', (string) $command)) {
                $command .= " LEFT JOIN site_urls AS 2su ON (2cs.click_referer_site_url_id = 2su.site_url_id) ";
            }

            if (! preg_match('/site_domains/', (string) $command)) {
                $command .= " LEFT JOIN site_domains AS 2sd ON (2su.site_domain_id = 2sd.site_domain_id) ";
            }
        ...
        if ($user_row['user_pref_keyword']) {
            if (! preg_match('/keywords/', (string) $command)) {
                $command .= " LEFT JOIN keywords AS 2k ON (2ca.keyword_id = 2k.keyword_id) ";
            }
        ...
        if ($user_row['user_pref_ip']) {
            if (! preg_match('/ips/', (string) $command)) {
                $command .= " LEFT JOIN ips AS 2i ON (2ca.ip_id = 2i.ip_id) ";
            }
            // Count query: no JOIN needed — use subquery (see count_where below)
        }

        // if there is a country lookup, and we have not joined the 202 country table. do so now
        if ($user_row['user_pref_country_id'] and ! preg_match('/locations_country/', (string) $command)) {
            $command .= " LEFT JOIN locations_country AS 2cy ON (2ca.country_id = 2cy.country_id) ";
        }

        // if there is a region lookup, and we have not joined the 202 region table. do so now
        if ($user_row['user_pref_region_id'] and ! preg_match('/locations_region/', (string) $command)) {
            $command .= " LEFT JOIN locations_region AS 2rg ON (2ca.region_id = 2rg.region_id) ";
        }

        // if there is a isp lookup, and we have not joined the 202 isp table. do so now
        if ($user_row['user_pref_isp_id'] and ! preg_match('/locations_isp/', (string) $command)) {
            $command .= " LEFT JOIN locations_isp AS 2is ON (2ca.isp_id = 2is.isp_id) ";
        }

        // if there is a device lookup, and we have not joined the 202 device table. do so now
        if ($user_row['user_pref_device_id']) {
            if (! preg_match('/device_models/', (string) $command)) {
                $command .= " LEFT JOIN device_models AS 2d ON (2ca.device_id = 2d.device_id) ";
            }
            // Count query: no JOIN needed — use subquery (see count_where below)
        }

        // if there is a browser lookup, and we have not joined the 202 browser table. do so now
        if ($user_row['user_pref_browser_id'] and ! preg_match('/browsers/', (string) $command)) {
            $command .= " LEFT JOIN browsers AS 2b ON (2ca.browser_id = 2b.browser_id) ";
        }

        // if there is a platform lookup, and we have not joined the 202 platform table. do so now
        if ($user_row['user_pref_platform_id'] and ! preg_match('/platforms/', (string) $command)) {
            $command .= " LEFT JOIN platforms AS 2p ON (2ca.platform_id = 2p.platform_id) ";
        }
    }
    ...
    if ($pref_adv == true) {
        if ($user_row['user_pref_ppc_network_id'] and ! ($user_row['user_pref_ppc_account_id'])) {
            ...
            if ($user_row['user_pref_ppc_network_id'] == '16777215') {
                ...
                $count_where .= "  AND      2c.ppc_account_id IN (SELECT ppc_account_id FROM ppc_accounts WHERE ppc_network_id='" . $mysql['user_pref_ppc_network_id'] . "')";
            }
        }
    ...
    if (stripos($click_sql, "dataengine")) {
...
function pcc_network_icon($ppc_network_name, $ppc_account_name)
{
    ...
    $html['ppc_network_icon'] = '<img src="' . get_absolute_url() . 'img/icons/ppc/' . $ppc_network_icon . '" width="16" height="16" alt="' . $ppc_network_name . '" title="' . $ppc_network_name . ': ' . $ppc_account_name . '"/>';
...
class INDEXES
{
    ...
    public static function get_ip_id($ip)
    {
        ...
        if ($ipType === 'ipv6' && $inet6_aton !== '') {
            $ip_sql = 'SELECT  ips.ip_id FROM ips_v6  INNER JOIN ips on (ips_v6.ip_id = ips.ip_address COLLATE utf8mb4_general_ci) WHERE ips_v6.ip_address= ' . $inet6_aton . '("' . $mysql['ip_address'] . '") order by ips.ip_id DESC limit 1';
        } else {
...
function  upgrade_config()
{
    ...
    if (!file_exists('config-sample.php')) {
        //_die('Sorry, I need a config-sample.php file to work from. Please re-upload this file from your 1ai-Affiliate installation.');
        return;
    }
    ...
    if (!is_writable(substr(__DIR__, 0, -10) . '/')) {
        // _die("Sorry your config.php needs upgrading but I can't update it automatically because I can't write to the directory.<br>You'll have to either change the permissions on your 1ai-Affiliate directory or create your config.php manually by copying from config-sample.php.");
        return;
    }
