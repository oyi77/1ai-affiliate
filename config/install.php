...
if (isset($_COOKIE['user_api'])) {
	...
	header("Location: " . get_absolute_url() . "config/get_apikey.php");
	exit;
}
...
if (is_installed() == true) {
	...
	_die("<h6>Already Installed</h6>
			  <small>You appear to have already installed 1ai-Affiliate. To reinstall please clear your old database tables first. <a href='" . get_absolute_url() . "login.php'>Login Now</a></small>");
}
...
if (!$success) {
	...
	if ($version_error) {
		header("Location: /config/requirements.php");
	}
...
if ($success) {
	...
	<div class="main col-xs-7 install">
		<center><img src="<?php echo get_absolute_url(); ?>img/oneai_affiliate.png"></center>
		<h6>Success!</h6>
		<small>1ai-Affiliate has been installed. Now you can <a href="<?php echo get_absolute_url(); ?>login.php">log in</a>.</small><br></br>
		...
		<div class="row" style="margin-bottom: 10px;">
			...
			<div class="col-xs-9"><small><?php printf('<a href="%slogin.php">%slogin.php</a>', get_absolute_url(), $_SERVER['SERVER_NAME'] . get_absolute_url()); ?></small></div>
		</div>
