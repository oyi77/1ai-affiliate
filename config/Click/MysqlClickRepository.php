...
final class MysqlClickRepository implements ClickRepositoryInterface
{
    ...
    public function allocateClickId(): int
    {
        $stmt = $this->conn->prepareWrite(
            'INSERT INTO clicks_counter SET click_id = DEFAULT'
        );
    ...
    public function recordClick(ClickRecord $click): int
    {
        return $this->conn->transaction(function () use ($click): int {
            ...
            // 2. clicks — core click data
            $stmt = $this->conn->prepareWrite(
                'INSERT INTO clicks SET
            ...
            // 3. clicks_variable
            $stmt = $this->conn->prepareWrite(
                'INSERT INTO clicks_variable (click_id, variable_set_id) VALUES (?, ?)'
            );
            $this->conn->bind($stmt, 'ii', [$clickId, $click->variableSetId]);
            $this->conn->execute($stmt);
            $stmt->close();

            // 4. google — UTM and gclid data
            $stmt = $this->conn->prepareWrite(
                'INSERT INTO google SET
                    click_id = ?, gclid = ?,
            ...
            // 5. clicks_spy — denormalized copy of core click data
            $stmt = $this->conn->prepareWrite(
                'INSERT INTO clicks_spy SET
                    click_id = ?, user_id = ?, aff_campaign_id = ?,
                    landing_page_id = ?, ppc_account_id = ?,
                    click_cpc = ?, click_payout = ?,
                    click_filtered = ?, click_bot = ?,
            ...
            // 6. clicks_advance — geo/device/keyword data
            $stmt = $this->conn->prepareWrite(
                'INSERT INTO clicks_advance SET
            ...
            // 7. clicks_tracking — c1-c4 tracking params
            $stmt = $this->conn->prepareWrite(
                'INSERT INTO clicks_tracking SET
            ...
            // 8. clicks_record — public ID and cloaking state
            $stmt = $this->conn->prepareWrite(
                'INSERT INTO clicks_record SET
            ...
            // 9. clicks_site — URL references
            $stmt = $this->conn->prepareWrite(
                'INSERT INTO clicks_site SET
                    click_id = ?,
