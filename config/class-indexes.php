...
class INDEXES
{
    ...
    public static function get_city_id($city_name, $country_id)
    {
        ...
        if ($memcacheWorking) {
            ...
            if ($getID) {
                ...
                $city_sql = "SELECT city_id FROM locations_city WHERE city_name='" . $mysql['city_name'] . "' AND country_id='" . $mysql['country_id'] . "'";
                ...
                if ($city_row['city_id']) {
                    ...
                    //insert city
                    $city_sql = "INSERT INTO locations_city SET city_name='" . $mysql['city_name'] . "', country_id='" . $mysql['country_id'] . "'";
                    $city_result = _mysqli_query($city_sql);
    ...
    public static function get_isp_id($isp)
    {
        ...
        if ($memcacheWorking) {
            ...
            if ($getID) {
                ...
                $isp_sql = "SELECT isp_id FROM locations_isp WHERE isp_name='" . $mysql['isp_name'] . "'";
                ...
                if ($isp_row['isp_id']) {
                    ...
                    //insert isp
                    $isp_sql = "INSERT INTO locations_isp SET isp_name='" . $mysql['isp_name'] . "'";
                    $isp_result = _mysqli_query($isp_sql);
                    $isp_id = $db->insert_id;
                    // add to memcached
                    $setID = setCache(md5("isp-id" . $mysql['isp_name'] . systemHash()), $isp_id, $time);
    ...
    public static function insert_ip($ip_address_string)
    {
        ...
        if ($ip_type === 'ipv6') {
            ...
            $ip_sql_v6 = "INSERT INTO ips_v6 SET ip_address = '" . $escaped_packed_ip . "'";
            $ip_result_v6 = _mysqli_query($ip_sql_v6); // Correct call: uses global $db

            if ($ip_result_v6) {
                $ipv6_ref_id = $db->insert_id; // Get the ID from ips_v6 table

                // Insert a reference into ips (adjust based on schema relationship)
                // Assuming ips.ip_address stores the reference ID for v6
                $ip_sql_ref = "INSERT INTO ips SET ip_address = '" . $db->real_escape_string($ipv6_ref_id) . "' "; // You might need a flag column to indicate this is a v6 reference
                $ip_result_ref = _mysqli_query($ip_sql_ref); // Correct call

                if ($ip_result_ref) {
                    $ip_id = $db->insert_id; // This is the final ip_id from ips
                } else {
                    try {
                        error_log("Failed to insert IPv6 reference into ips: " . $db->error);
                    } catch (\Error $e) {
                        error_log("Failed to insert IPv6 reference into ips (error inaccessible)");
                    }
                }
            } else {
                try {
                    error_log("Failed to insert IPv6 address into ips_v6: " . $db->error);
                } catch (\Error $e) {
                    error_log("Failed to insert IPv6 address into ips_v6 (error inaccessible)");
                }
            }
            return $ip_id;
        } else { // IPv4
            ...
            if ($ip_result) {
                ...
                try {
                    error_log("Failed to insert IPv4 address into ips: " . $db->error);
                } catch (\Error $e) {
                    error_log("Failed to insert IPv4 address into ips (error inaccessible)");
                }
            }
            return $ip_id;
        }
    ...
    public static function get_site_domain_id($site_url)
    {
        ...
        $site_domain_sql = "SELECT site_domain_id FROM site_domains WHERE site_domain='" . $mysql['site_domain'] . "'";
        $site_domain_result = $db->query($site_domain_sql); // or record_mysql_error($site_domain_sql);

        if ($site_domain_result->num_rows == 0) {
            $site_domain_sql = "INSERT INTO site_domains SET site_domain='" . $mysql['site_domain'] . "'";
    ...
    public static function get_site_url_id($site_url)
    {
        ...
        $site_url_sql = "SELECT site_url_id FROM site_urls WHERE site_url='" . $mysql['site_url'] . "'";
        ...
        if ($site_url_result->num_rows == 0) {
            ...
            $site_url_sql = "INSERT INTO site_urls SET site_url='" . $mysql['site_url'] . "', site_domain_id='" . $mysql['site_domain_id'] . "'";
    ...
    public static function get_keyword_id($keyword_name)
    {
        ...
        $keyword_sql = "SELECT keyword_id FROM keywords WHERE keyword_name='" . $mysql['keyword_name'] . "'";
        $keyword_result = $db->query($keyword_sql); // or record_mysql_error($keyword_sql);

        if ($keyword_result->num_rows == 0) {
            $keyword_sql = "INSERT INTO keywords SET keyword_name='" . $mysql['keyword_name'] . "'";
    ...
    public static function get_c1_id($c1)
    {
        ...
        if ($memcacheWorking) {
            ...
            if ($getID) {
                ...
                $c1_sql = "SELECT c1_id FROM clicks_c1 WHERE c1='" . $mysql['c1'] . "'";
                ...
                if ($c1_row['c1_id']) {
                    ...
                    //insert c1
                    $c1_sql = "INSERT INTO clicks_c1 SET c1='" . $mysql['c1'] . "'";
                    $c1_result = _mysqli_query($c1_sql);
                    $c1_id = $db->insert_id;
                    // add to memcached
                    $setID = setCache(md5("c1-id" . $c1 . systemHash()), $c1_id, $time);
    ...
    public static function get_c2_id($c2)
    {
        ...
        if ($memcacheWorking) {
            ...
            if ($getID) {
                ...
                $c2_sql = "SELECT c2_id FROM clicks_c2 WHERE c2='" . $mysql['c2'] . "'";
                ...
                if ($c2_row['c2_id']) {
                    ...
                    //insert c2
                    $c2_sql = "INSERT INTO clicks_c2 SET c2='" . $mysql['c2'] . "'";
                    $c2_result = _mysqli_query($c2_sql);
                    $c2_id = $db->insert_id;
                    // add to memcached
                    $setID = setCache(md5("c2-id" . $c2 . systemHash()), $c2_id, $time);
    ...
    public static function get_c3_id($c3)
    {
        ...
        if ($memcacheWorking) {
            ...
            if ($getID) {
                ...
                $c3_sql = "SELECT c3_id FROM clicks_c3 WHERE c3='" . $mysql['c3'] . "'";
                ...
                if ($c3_row['c3_id']) {
                    ...
                    //insert c3
                    $c3_sql = "INSERT INTO clicks_c3 SET c3='" . $mysql['c3'] . "'";
                    $c3_result = _mysqli_query($c3_sql);
                    $c3_id = $db->insert_id;
                    // add to memcached
                    $setID = setCache(md5("c3-id" . $c3 . systemHash()), $c3_id, $time);
    ...
    public static function get_c4_id($c4)
    {
        ...
        if ($memcacheWorking) {
            ...
            if ($getID) {
                ...
                $c4_sql = "SELECT c4_id FROM clicks_c4 WHERE c4='" . $mysql['c4'] . "'";
                ...
                if ($c4_row['c4_id']) {
                    ...
                    //insert c4
                    $c4_sql = "INSERT INTO clicks_c4 SET c4='" . $mysql['c4'] . "'";
                    $c4_result = _mysqli_query($c4_sql);
                    $c4_id = $db->insert_id;
                    // add to memcached
                    $setID = setCache(md5("c4-id" . $c4 . systemHash()), $c4_id, $time);
    ...
    public static function get_browser_id($browser_name)
    {
        ...
        $browser_sql = "SELECT browser_id FROM browsers WHERE browser_name='" . $mysql['browser_name'] . "'";
        $browser_result = $db->query($browser_sql); // or record_mysql_error($browser_sql);

        if ($browser_result->num_rows == 0) {
            $browser_sql = "INSERT INTO browsers SET browser_name='" . $mysql['browser_name'] . "'";
    ...
    public static function get_platform_id($platform_name)
    {
        ...
        $platform_sql = "SELECT platform_id FROM platforms WHERE platform_name='" . $mysql['platform_name'] . "'";
        $platform_result = $db->query($platform_sql); // or record_mysql_error($platform_sql);

        if ($platform_result->num_rows == 0) {
            $platform_sql = "INSERT INTO platforms SET platform_name='" . $mysql['platform_name'] . "'";
    ...
    public static function get_device_id($device_name)
    {
        ...
        $device_sql = "SELECT device_id FROM devices WHERE device_name='" . $mysql['device_name'] . "'";
        $device_result = $db->query($device_sql); // or record_mysql_error($device_sql);

        if ($device_result->num_rows == 0) {
            $device_sql = "INSERT INTO devices SET device_name='" . $mysql['device_name'] . "'";
    ...
    public static function get_region_id($region_name, $country_id)
    {
        ...
        if ($memcacheWorking) {
            ...
            if ($getID) {
                ...
                $region_sql = "SELECT region_id FROM locations_region WHERE region_name='" . $mysql['region_name'] . "' AND country_id='" . $mysql['country_id'] . "'";
                ...
                if ($region_row['region_id']) {
                    ...
                    $region_sql = "INSERT INTO locations_region SET region_name='" . $mysql['region_name'] . "', country_id='" . $mysql['country_id'] . "'";
                    $region_result = _mysqli_query($region_sql);
