...
try {
    ...
    _die("<h6>Error establishing a database connection</h6>
			<p><small>This either means that the username and password information in your <code>config.php</code> file is incorrect or we can't contact the database server. This could mean your host's database server is down.</small></p>
...
if (($navigation[1]) and ($navigation[1] != 'config')) {
    ...
    //	include_once(ROOT_PATH . '/cronjobs/index.php'); 
}
...
if ($skip_upgrade == false) {
    ...
    if (is_installed() == true) {
        ...
        if ((upgrade_needed() == true) and (($navigation[1] != 'config') and (($navigation[2] ?? '') != 'upgrade.php'))) {
...
if (!str_contains((string) $_SERVER['PHP_SELF'], 'config/install.php')) {
...
function updateBlazerCache(&$mysql, $blazerCacheType = '')
{
    ...
    switch ($blazerCacheType) {
        case 'ac':
            $bc_results = ($db->query("SELECT DISTINCT tracker_id_public from trackers WHERE aff_campaign_id = 1"));
