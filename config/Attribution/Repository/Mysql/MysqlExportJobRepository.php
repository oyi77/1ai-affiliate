...
final readonly class MysqlExportJobRepository implements ExportJobRepositoryInterface
{
    ...
    {
        $sql = "SELECT * FROM attribution_exports WHERE status = 'pending' ORDER BY queued_at ASC LIMIT ?";
        $stmt = $this->conn->prepareRead($sql);
        $limit = max(1, $limit);
    ...
    {
        $sql = "UPDATE attribution_exports SET status = 'processing', started_at = ?, updated_at = ? WHERE export_id = ? LIMIT 1";
        $stmt = $this->conn->prepareWrite($sql);
    ...
    {
        $sql = "UPDATE attribution_exports SET status = 'completed', file_path = ?, rows_exported = ?, completed_at = ?, updated_at = ?, last_error = NULL WHERE export_id = ? LIMIT 1";
        $stmt = $this->conn->prepareWrite($sql);
    ...
    {
        $sql = "UPDATE attribution_exports SET status = 'failed', failed_at = ?, updated_at = ?, last_error = ? WHERE export_id = ? LIMIT 1";
        $stmt = $this->conn->prepareWrite($sql);
    ...
    {
        $sql = 'UPDATE attribution_exports SET webhook_attempted_at = ?, webhook_status_code = ?, webhook_response_body = ?, updated_at = ? WHERE export_id = ? LIMIT 1';
        $stmt = $this->conn->prepareWrite($sql);
    ...
    {
        $sql = 'SELECT * FROM attribution_exports WHERE user_id = ? AND model_id = ? ORDER BY queued_at DESC LIMIT ?';
        $stmt = $this->conn->prepareRead($sql);
        $limit = max(1, $limit);
