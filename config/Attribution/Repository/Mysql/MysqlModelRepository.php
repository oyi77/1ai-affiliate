...
final readonly class MysqlModelRepository implements ModelRepositoryInterface
{
    ...
    {
        $sql = 'SELECT * FROM attribution_models WHERE model_id = ? LIMIT 1';
    ...
    {
        $sql = 'SELECT * FROM attribution_models WHERE user_id = ? AND is_default = 1 LIMIT 1';
        $stmt = $this->conn->prepareRead($sql);
    ...
    {
        $sql = 'SELECT * FROM attribution_models WHERE user_id = ?';
        $types = 'i';
        $values = [$userId];
    ...
    {
        $sql = 'SELECT * FROM attribution_models WHERE user_id = ? AND model_slug = ? LIMIT 1';
        $stmt = $this->conn->prepareRead($sql);
    ...
    public function save(ModelDefinition $model): ModelDefinition
    {
        ...
        if ($model->modelId === null) {
            $sql = 'INSERT INTO attribution_models (user_id, model_name, model_slug, model_type, weighting_config, is_active, is_default, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)';
        ...
        $sql = 'UPDATE attribution_models SET model_name = ?, model_slug = ?, model_type = ?, weighting_config = ?, is_active = ?, is_default = ?, created_at = ?, updated_at = ? WHERE model_id = ? LIMIT 1';
        $stmt = $this->conn->prepareWrite($sql);
    ...
    public function promoteToDefault(ModelDefinition $model): void
    {
        ...
        $this->conn->transaction(function () use ($userId, $modelId): void {
            $stmtReset = $this->conn->prepareWrite('UPDATE attribution_models SET is_default = 0 WHERE user_id = ?');
            $this->conn->bind($stmtReset, 'i', [$userId]);
            $this->conn->execute($stmtReset);
            $stmtReset->close();

            $stmtSet = $this->conn->prepareWrite('UPDATE attribution_models SET is_default = 1, is_active = 1 WHERE model_id = ? LIMIT 1');
    ...
    public function setAsDefault(int $userId, int $modelId): bool
    {
        return $this->conn->transaction(function () use ($userId, $modelId): bool {
            ...
            $checkStmt = $this->conn->prepareWrite('SELECT 1 FROM attribution_models WHERE model_id = ? AND user_id = ? LIMIT 1');
            ...
            $resetStmt = $this->conn->prepareWrite('UPDATE attribution_models SET is_default = 0 WHERE user_id = ?');
            $this->conn->bind($resetStmt, 'i', [$userId]);
            $this->conn->execute($resetStmt);
            $resetStmt->close();

            // Set the specified model as default (and activate it)
            $setStmt = $this->conn->prepareWrite('UPDATE attribution_models SET is_default = 1, is_active = 1 WHERE model_id = ? LIMIT 1');
    ...
    public function delete(int $modelId, int $userId): void
    {
        $this->conn->transaction(function () use ($modelId, $userId): void {
            $stmt = $this->conn->prepareWrite('DELETE tp FROM attribution_touchpoints tp INNER JOIN attribution_snapshots s ON tp.snapshot_id = s.snapshot_id WHERE s.model_id = ?');
            $this->conn->bind($stmt, 'i', [$modelId]);
            $this->conn->execute($stmt);
            $stmt->close();

            $stmt = $this->conn->prepareWrite('DELETE FROM attribution_snapshots WHERE model_id = ?');
            $this->conn->bind($stmt, 'i', [$modelId]);
            $this->conn->execute($stmt);
            $stmt->close();

            $stmt = $this->conn->prepareWrite('DELETE FROM attribution_settings WHERE model_id = ?');
            $this->conn->bind($stmt, 'i', [$modelId]);
            $this->conn->execute($stmt);
            $stmt->close();

            $stmt = $this->conn->prepareWrite('DELETE FROM attribution_models WHERE model_id = ? AND user_id = ?');
