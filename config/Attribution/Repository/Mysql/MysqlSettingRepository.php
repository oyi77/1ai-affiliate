...
final class MysqlSettingRepository implements SettingsRepositoryInterface
{
    ...
    public function findForScopes(int $userId, array $scopes): array
    {
        ...
        $sql = 'SELECT * FROM attribution_settings WHERE user_id = ? AND (' . implode(' OR ', $parts) . ')';
    ...
    public function save(Setting $setting): Setting
    {
        ...
        if ($settingId === null) {
            ...
            $sql = 'INSERT INTO attribution_settings (' . implode(', ', $columns) . ') VALUES (' . implode(', ', $placeholders) . ')';
        ...
        $sql = 'UPDATE attribution_settings SET ' . implode(', ', $sets) . ' WHERE setting_id = ? LIMIT 1';
        $stmt = $this->conn->prepareWrite($sql);
        $this->conn->bind($stmt, $types, $params);
    ...
    {
        $sql = 'SELECT * FROM attribution_settings WHERE multi_touch_enabled = 0';
        $stmt = $this->conn->prepareRead($sql);
        $rows = $this->conn->fetchAll($stmt);
    ...
    {
        $stmt = $this->conn->prepareRead('SELECT * FROM attribution_settings WHERE setting_id = ? LIMIT 1');
