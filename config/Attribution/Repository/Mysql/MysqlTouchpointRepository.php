...
final readonly class MysqlTouchpointRepository implements TouchpointRepositoryInterface
{
    ...
    {
        $sql = 'SELECT * FROM attribution_touchpoints WHERE snapshot_id = ? ORDER BY position ASC, touchpoint_id ASC';
    ...
    public function saveBatch(array $touchpoints): void
    {
        ...
        $this->conn->transaction(function () use ($touchpoints): void {
            $sql = 'INSERT INTO attribution_touchpoints (snapshot_id, conv_id, click_id, position, credit, weight, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)';
    ...
    {
        $sql = 'DELETE FROM attribution_touchpoints WHERE snapshot_id = ?';
        $stmt = $this->conn->prepareWrite($sql);
        $this->conn->bind($stmt, 'i', [$snapshotId]);
