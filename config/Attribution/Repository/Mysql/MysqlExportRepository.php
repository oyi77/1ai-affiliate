...
final readonly class MysqlExportRepository implements ExportRepositoryInterface
{
    ...
    {
        $sql = 'INSERT INTO attribution_exports (user_id, model_id, scope_type, scope_id, start_hour, end_hour, format, status, file_path, download_token, webhook_url, webhook_method, webhook_headers, webhook_status_code, webhook_response_body, last_attempted_at, completed_at, error_message, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
    ...
    {
        $sql = 'UPDATE attribution_exports SET status = ?, file_path = ?, download_token = ?, webhook_url = ?, webhook_method = ?, webhook_headers = ?, webhook_status_code = ?, webhook_response_body = ?, last_attempted_at = ?, completed_at = ?, error_message = ?, updated_at = ? WHERE export_id = ? LIMIT 1';
        $stmt = $this->conn->prepareWrite($sql);
    ...
    {
        $sql = 'SELECT * FROM attribution_exports WHERE export_id = ? LIMIT 1';
    ...
    public function findForUser(int $userId, ?int $modelId = null, int $limit = 25): array
    {
        ...
        $sql = 'SELECT * FROM attribution_exports WHERE user_id = ?';
        $types = 'i';
        $params = [$userId];
    ...
    public function claimPending(int $limit = 10): array
    {
        ...
        $now = time();
        $sql = 'SELECT * FROM attribution_exports WHERE status = ? ORDER BY created_at ASC LIMIT ?';
        $stmt = $this->conn->prepareWrite($sql);
        ...
        foreach ($rows as $row) {
            ...
            $update = $this->conn->prepareWrite('UPDATE attribution_exports SET status = ?, last_attempted_at = ?, updated_at = ? WHERE export_id = ? AND status = ? LIMIT 1');
