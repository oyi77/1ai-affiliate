...
class AttributionIntegrationService
{
    ...
    public function updateCampaignAttributionModel(int $campaignId, ?int $modelId, int $userId): bool
    {
        ...
        $campaignSql = "SELECT aff_campaign_id FROM aff_campaigns WHERE aff_campaign_id = ? AND user_id = ? LIMIT 1";
        ...
        // Update the campaign
        $updateSql = "UPDATE aff_campaigns SET attribution_model_id = ? WHERE aff_campaign_id = ? AND user_id = ? LIMIT 1";
    ...
    public function getAttributionModelStats(int $userId): array
    {
        $sql = "SELECT 
                ...
                LEFT JOIN aff_campaigns ac ON am.model_id = ac.attribution_model_id
