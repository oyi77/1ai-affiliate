...
namespace OneAIAffiliate\Conversion;
...
final class MysqlConversionRepository implements ConversionRepositoryInterface
{
    ...
    public function create(int $userId, array $data): int
    {
        ...
        return $this->conn->transaction(function () use ($clickId, $transactionId, $payoutOverride, $userId, $convTime): int {
            ...
            $clickStmt = $this->conn->prepareWrite(
                'SELECT click_id, aff_campaign_id, click_payout, click_time FROM clicks WHERE click_id = ? AND user_id = ? LIMIT 1 FOR UPDATE'
            );
            ...
            $stmt = $this->conn->prepareWrite(
                'UPDATE clicks SET click_lead = 1, click_payout = ? WHERE click_id = ? AND user_id = ?'
            );
    ...
    {
        $stmt = $this->conn->prepareWrite(
            'UPDATE conversion_logs SET deleted = 1 WHERE conv_id = ? AND user_id = ?'
        );
        $this->conn->bind($stmt, 'ii', [$id, $userId]);
        $this->conn->execute($stmt);
        $stmt->close();
