...
if (!file_exists(substr(__DIR__, 0, -10) . '/config-sample.php')) {
...
$configFile = file(substr(__DIR__, 0, -10) . '/config-sample.php');


//check to see if the directory is writable
if (!is_writable(substr(__DIR__, 0, -10) . '/')) {
	_die("Sorry, I can't write to the directory. You'll have to either change the permissions on your 1ai-Affiliate directory or create your config.php manually.");
}


// Check if config.php has been created
if (file_exists('../config.php')) {
	//_die("<p>The file 'config.php' already exists. If you need to reset any of the configuration items in this file, please delete it first. You may try <a href='install.php'>installing now</a>.</p>");
	$re = '/(\$\w*) = \'(\w*)\';/i';

	$handle = fopen(substr(__DIR__, 0, -10) . '/config.php', 'r');
	$old = [];
	while (!feof($handle)) {
		$line = fgets($handle);
		if ($line === false) {
...
?>
		<div class="main col-xs-7 install">
			...
			<small><strong>If for any reason this automatic file creation doesn't work, don't worry. All this does is fill in the database information to a configuration file. You may also simply open <code>config-sample.php</code> in a text editor, fill in your information, and save it as <code>config.php</code>. </strong>
				<br></br>
	...
	?>
		...
		if ($step == 1) {
			...
			$msg = "Your config.php needs to be updated to a new format. Please review the settings we got from your old file and make changes as needed";
...
<?php
	...
	case 2.2:
		...
		$handle = fopen(substr(__DIR__, 0, -10) . '/config.php', 'w');
