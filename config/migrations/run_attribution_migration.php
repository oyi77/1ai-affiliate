...
try {
    ...
    $result = $db->query("SELECT user_id, model_name, model_type, is_default FROM attribution_models WHERE is_default = 1 ORDER BY user_id LIMIT 10");
    ...
    if ($result && $result->num_rows > 0) {
        ...
        if ($result->num_rows >= 10) {
            $totalResult = $db->query("SELECT COUNT(*) as total FROM attribution_models WHERE is_default = 1");
