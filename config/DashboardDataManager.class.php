...
class DashboardDataManager
{
    ...
    private function storeContent(string $contentType, array $data): bool
    {
        ...
        try {
            ...
            $sql = "DELETE FROM dashboard_content WHERE content_type = ?";
            ...
            // Insert new content
            $insertSql = "INSERT INTO dashboard_content 
    ...
    private function updateSyncStatus(string $contentType, string $status, ?string $error = null): void
    {
        ...
        if ($status === 'start') {
            ...
            $sql = "INSERT INTO dashboard_sync (content_type, last_sync) 
                    VALUES (?, ?) 
            ...
            $sql = "INSERT INTO dashboard_sync (content_type, last_success, error_count) 
                    VALUES (?, ?, 0) 
            ...
            $sql = "INSERT INTO dashboard_sync (content_type, error_count, last_error) 
                    VALUES (?, 1, ?) 
                    ON DUPLICATE KEY UPDATE 
    ...
    public function getSyncStatus(string $contentType): ?array
    {
        ...
        $sql = "SELECT * FROM dashboard_sync WHERE content_type = ?";
        $stmt = self::$db->prepare($sql);
        if (!$stmt) {
            return null;
        }
