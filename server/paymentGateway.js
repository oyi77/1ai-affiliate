...
// ── Handle Callbacks (with signature verification) ──────────────

/**
 * Verify Tripay callback signature.
 * Tripay sends signature in X-Callback-Signature header.
 * Signature = HMAC-SHA256(privateKey, JSON(body))
 */
function verifyTripaySignature(body, headers) {
  const privateKey = gateways.tripay.config.privateKey;
  if (!privateKey) return { valid: false, reason: 'No private key configured' };

  const signature = headers['x-callback-signature'] || headers['X-Callback-Signature'];
  if (!signature) return { valid: false, reason: 'No signature header' };
 ...
 * Midtrans sends SHA-512 hash of order_id+status_code+gross_amount+serverKey
 */
function verifyMidtransSignature(body) {
  const serverKey = gateways.midtrans.config.serverKey;
  if (!serverKey) return { valid: false, reason: 'No server key configured' };

  const { order_id, status_code, gross_amount, signature_key } = body;
  if (!signature_key) return { valid: false, reason: 'No signature_key in body' };

  const expected = crypto.createHash('sha512')
    .update(`${order_id}${status_code}${gross_amount}${serverKey}`)
    .digest('hex');

  if (signature_key !== expected) return { valid: false, reason: 'Signature mismatch' };
  return { valid: true };
}

/**
 * Verify NowPayments IPN signature.
 * NowPayments sends X-Nowpayments-Sig header = HMAC-SHA512(ipnSecret, JSON(body))
 */
function verifyNowPaymentsSignature(body, headers) {
  const ipnSecret = gateways.nowpayments.config.ipnSecret;
...
async function handleTripayCallback(body, headers) {
  // Verify signature
  const verification = verifyTripaySignature(body, headers);
  if (!verification.valid) {
    console.error('Tripay callback signature invalid:', verification.reason);
    return { success: false, error: 'Invalid signature' };
  }

  const { reference, status } = body;
  if (status === 'PAID') {
    // Prevent double-credit: check current status first
    const [[payment]] = await pool.query('SELECT status FROM 1ai_payments WHERE reference = ?', [reference]);
...
async function handleMidtransCallback(body) {
  // Verify signature
  const verification = verifyMidtransSignature(body);
  if (!verification.valid) {
    console.error('Midtrans callback signature invalid:', verification.reason);
    return { success: false, error: 'Invalid signature' };
  }
  ...
  if (transaction_status === 'capture' || transaction_status === 'settlement') {
    ...
    const [[payment]] = await pool.query('SELECT status FROM 1ai_payments WHERE reference = ?', [order_id]);
    if (!payment || payment.status === 'paid') {
      return { success: true, already_paid: true };
    }
...
async function handleNowPaymentsCallback(body, headers) {
  // Verify signature
  const verification = verifyNowPaymentsSignature(body, headers);
  if (!verification.valid) {
    console.error('NowPayments callback signature invalid:', verification.reason);
    return { success: false, error: 'Invalid signature' };
  }
