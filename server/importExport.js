...
async function generateROASReport(date_from, date_to) {
  ...
  const [spendRows] = await pool.query(
    `SELECT date, source, campaign_name, SUM(spend) as spend, SUM(clicks) as clicks, SUM(impressions) as impressions
  ...
  const [convRows] = await pool.query(
    `SELECT DATE(FROM_UNIXTIME(created_at)) as date, COUNT(*) as conversions, SUM(revenue) as revenue, SUM(payout) as commission
     FROM 1ai_conversions
  ...
  for (const s of spendRows) {
    ...
    dateMap[key].sources.push({ source: s.source, campaign: s.campaign_name, spend: parseFloat(s.spend || 0) });
  }
