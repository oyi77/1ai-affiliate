...
const pack = require('./wf02_content_2026-09-08.js');

(async () => {
  console.log('=== WF02 AFFILIATE BUZZER EXECUTION ===');
  const now = Math.floor(Date.now() / 1000);
  const slug = 'miniac-wf02-20260908';
  const linkToken = crypto.randomBytes(16).toString('hex');
  const trackingUrl = 'https://shopee.co.id/search?keyword=kipas+ac+portable+air+es+spray&sortBy=sales&utm_source=aff_wf02&utm_medium=affiliate&utm_campaign=wf02-miniac-20260908';

  let offerId, linkId;
  const con = await pool.getConnection();
  try {
    // Check if offer already created today
    ...
    if (existingOffer.length > 0) {
      ...
      const [oir] = await con.query(
        `INSERT INTO 1ai_offers (name, vertical, geo, type, payout, network_payout, payout_currency, status, approval_status, notes, created_at, updated_at, postback_enabled, tracking_url)
         VALUES (?, 'Home / Gadget', 'ID', 'CPA', 4500, 4500, 'IDR', 'active', 'approved', 'WF02 — Mini Air Cooler AC Portable', ?, ?, 1, ?)`,
  ...
  fs.writeFileSync('/home/openclaw/.hermes/workspace/hermes_vault/wf02/run-2026-09-08-wf02.json', JSON.stringify(output, null, 2));
