...
const pack = require('./wf02_content_2026-09-13.js');

(async () => {
  console.log('=== WF02 AFFILIATE BUZZER EXECUTION (2026-09-13) ===');
  const now = Math.floor(Date.now() / 1000);
  const slug = 'ac-portable-mini-spray-wf02-mtz64kuq';
  const linkToken = 'ba9cc9ea913533ac97ffd9fb76a5d975';
  const trackingUrl = 'https://shopee.co.id/search?keyword=kipas+ac+portable+humidifier+spray&sortBy=sales&utm_source=aff_wf02&utm_medium=affiliate&utm_campaign=wf02-acportable-20260913';

  let offerId = 746;
  let linkId = 536;
  const con = await pool.getConnection();
  try {
    const [existingOffer] = await con.query('SELECT id FROM 1ai_offers WHERE id = ? LIMIT 1', [offerId]);
    if (existingOffer.length > 0) {
      console.log(`[DB] Offer #${offerId} verified in database.`);
    } else {
      const [oir] = await con.query(
        `INSERT INTO 1ai_offers (id, name, vertical, geo, type, payout, network_payout, payout_currency, status, approval_status, notes, created_at, updated_at, postback_enabled, tracking_url)
         VALUES (746, ?, 'Home & Appliance', 'ID', 'CPA', 4500, 4500, 'IDR', 'active', 'approved', 'WF02 — Kipas Angin AC Portable Spray Humidifier 3in1', ?, ?, 1, ?)`,
    ...
    if (existingLink.length > 0) {
      ...
      console.log(`[DB] Affiliate Link #${linkId} verified for slug ${slug}`);
    } else {
      ...
      linkId = lr.insertId;
      console.log(`[DB] Created Link #${linkId} for slug ${slug}`);
    }
  } catch (e) {
  ...
  for (const p of pack.fbPosts) {
    ...
    if (!token) {
      ...
      results.push({ page: p.page, persona: p.persona, error: 'no_token' });
      continue;
    }
    try {
  ...
  fs.writeFileSync('/home/openclaw/.hermes/workspace/hermes_vault/wf02/run-2026-09-13-wf02.json', JSON.stringify(output, null, 2));

  fs.mkdirSync('/home/openclaw/.hermes/workspace/wf02-output', { recursive: true });
  const scriptTxt = `PRODUCT: ${pack.tiktokScript.product}
PRICE: ${pack.tiktokScript.price_range}
TITLE: ${pack.tiktokScript.title}
DURATION: ${pack.tiktokScript.duration}
TARGET NICHE: ${pack.tiktokScript.target_niche}
HOOK: ${pack.tiktokScript.hook}

BEATS:
${pack.tiktokScript.beats.map((b, i) => `${i + 1}. ${b}`).join('\n')}

CAPTION TIKTOK:
${pack.tiktokScript.caption_tiktok}
`;
  fs.writeFileSync('/home/openclaw/.hermes/workspace/wf02-output/wf02_tiktok_review_2026-09-13.txt', scriptTxt);
