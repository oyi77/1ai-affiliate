...
(async () => {
  ...
  const slug = 'mug-pemanas-portabel-wf02-' + Date.now().toString(36);
  const linkToken = crypto.randomBytes(16).toString('hex');
  const trackingUrl = 'https://shopee.co.id/search?keyword=mug+pemanas+air+portabel&sortBy=sales&utm_source=aff_wf02&utm_medium=affiliate&utm_campaign=wf02-mug-20260813';
  ...
  try {
    const [oir] = await con.query(
      `INSERT INTO 1ai_offers (name, vertical, geo, type, payout, network_payout, payout_currency, status, approval_status, notes, created_at, updated_at, postback_enabled, tracking_url)
       VALUES (?, 'Home / Gadget', 'ID', 'CPA', 4000, 4000, 'IDR', 'active', 'approved', 'WF02 — Mug pemanas viral', ?, ?, 1, ?)`,
      ['Mug Listrik Pemanas Air Portabel [Shopee Affiliate]', now, now, trackingUrl]
    );
