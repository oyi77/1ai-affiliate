...
class ApiClient
{
    ...
    public static function fromConfig(Config $config): self
    {
        ...
        if ($url === '') {
            throw new \RuntimeException("No URL configured. Run: p1ai config:set-url <url>");
        }
        if ($key === '') {
            throw new \RuntimeException("No API key configured. Run: p1ai config:set-key <key>");
        }
