...
include_once(__DIR__ . '/config/DeviceDetect.php');
...
if (!is_installed()) {
    ...
    header('Location: ' . get_absolute_url() . 'config/setup-config.php');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	$login_log_stmt = $db->prepare('INSERT INTO users_log (user_name, user_pass, ip_address, login_time, login_success, login_error, login_server, login_session) VALUES (?, ?, ?, ?, ?, ?, ?, ?)');
	if ($login_log_stmt) {
	...
	if (empty($error) && $user_row) {
		...
		$update_stmt = $db->prepare('UPDATE users SET user_last_login_ip_id = ?, modal_status = ?, vip_perks_status = ? WHERE user_id = ?');
		if ($update_stmt) {
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<p>
						<a href="http://www.google.com/chrome/" target="_blank">Google Chrome <img src="../img/chrome.png"></a>
						<a href="http://www.mozilla.org/en-US/firefox/new/‎" target="_blank">Mozilla Firefox <img src="../img/firefox.png"></a>
						<a href="http://www.apple.com/safari" target="_blank">Safari (Mac OS X) <img src="../img/safari.png"></a>
					</p>
				</div>
