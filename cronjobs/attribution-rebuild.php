...
if (isset($options['user'])) {
    ...
    $result = $conn->query('SELECT user_id FROM users WHERE user_deleted = 0');
    if ($result) {
...
if ($connection instanceof mysqli) {
    $checkStmt = $connection->prepare('SELECT 1 FROM cronjobs WHERE cronjob_type = ? AND cronjob_time = ? LIMIT 1');
    if ($checkStmt) {
        ...
        $insertStmt = $connection->prepare('INSERT INTO cronjobs SET cronjob_type = ?, cronjob_time = ?');
        if ($insertStmt) {
        ...
        $cleanupStmt = $connection->prepare('DELETE FROM cronjobs WHERE cronjob_type = ? AND cronjob_time < ?');
        if ($cleanupStmt) {
...
if ($connection instanceof mysqli) {
    $timestamp = time();
    $connection->query('REPLACE INTO cronjob_logs (id, last_execution_time) VALUES (2, ' . $timestamp . ')');
}
