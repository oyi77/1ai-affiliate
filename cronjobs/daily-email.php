...
try {
	...
	$hash = "SELECT install_hash FROM users WHERE user_id = '1'";
	...
	$user_sql = 'SELECT user_email, user_daily_email FROM users LEFT JOIN users_pref USING (user_id) WHERE user_id = 1';
	...
	$sql_today = "SELECT 
			...
			FROM clicks AS 2c
			LEFT JOIN clicks_record AS 2cr USING (click_id)
			LEFT JOIN aff_campaigns AS 2ca USING (aff_campaign_id)
