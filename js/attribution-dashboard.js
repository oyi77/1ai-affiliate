(function (window, document, $, Highcharts) {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    var webhookHeadersInput = root.querySelector(
      '[data-role="webhook-headers"]'
    );
      ...
      kpiCards.forEach(function (card) {
        ...
        switch (key) {
          ...
          case "clicks":
            ...
            break;
          case "roi":
            metric.textContent = resolveRoi(
              typeof totals.roi === "number" ? totals.roi : null
            );
            break;
          default:
            metric.textContent = "–";
    ...
    function renderTouchpointMix(mix) {
      ...
      mix.forEach(function (item) {
        ...
        value.textContent =
          formatPercent(item.share) + " · " + item.touch_count + " touches";
      ...
      fetchJson(apiBase + "/metrics?" + params.toString())
        .then(function (payload) {
          if (!payload || payload.error || !payload.data) {
            handleError(
              "Unable to load analytics for the selected configuration."
            );
    ...
    function loadExports() {
      ...
      fetchJson(apiBase + "/models/" + state.modelId + "/exports")
        ...
        .catch(function (error) {
          displayExportFeedback(
            error.message || "Failed to load export jobs.",
            true
          );
        });
    }
    ...
    function renderExports(jobs) {
      ...
      jobs.forEach(function (job) {
        ...
        queuedCell.textContent = job.queued_at
          ? new Date(job.queued_at * 1000).toLocaleString()
          : "—";
        row.appendChild(queuedCell);

        var rowsCell = document.createElement("td");
        rowsCell.textContent =
          typeof job.rows_exported === "number" ? job.rows_exported : "—";
        row.appendChild(rowsCell);

        var completedCell = document.createElement("td");
        completedCell.textContent = job.completed_at
    ...
    function collectWebhookConfig() {
      var url =
        webhookUrlInput && webhookUrlInput.value
          ? webhookUrlInput.value.trim()
          : "";
      var secret =
        webhookSecretInput && webhookSecretInput.value
          ? webhookSecretInput.value.trim()
          : "";
      var headers = webhookHeadersInput
        ? parseWebhookHeaders(webhookHeadersInput.value)
        : {};
    ...
    function scheduleExport(format) {
      ...
      var startHour = exportStartInput
        ? fromDateTimeLocal(exportStartInput.value)
        : state.startHour;
      if (startHour === null) {
        startHour = state.startHour;
      }

      var endHour = exportEndInput
        ? fromDateTimeLocal(exportEndInput.value)
        : state.endHour;
      if (endHour === null) {
      ...
      if (startHour === null || endHour === null) {
        displayExportFeedback(
          "Provide valid start and end timestamps for the export.",
          true
        );
        return;
      }
      ...
      if (startHour >= endHour) {
        displayExportFeedback(
          "Start time must be before the end time for exports.",
          true
        );
        return;
      }
      ...
      postJson(apiBase + "/models/" + state.modelId + "/exports", payload)
        .then(function (response) {
          ...
          displayExportFeedback(
            "Export queued successfully. Refresh the jobs list for updates.",
            false
          );
          loadExports();
        })
        .catch(function (error) {
          displayExportFeedback(
            error.message || "Failed to schedule export.",
            true
          );
        });
    }
      ...
      fetchJson(apiBase + "/models")
        .then(function (payload) {
          ...
          if (data.length === 0) {
            ...
            handleError(
              "Create an attribution model to begin analysing journeys."
            );
            return;
          }
