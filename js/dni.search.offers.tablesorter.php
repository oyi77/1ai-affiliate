...
$(function() {
    ...
    tablesorterPagerOptions = {
            ...
            ajaxProcessing: function(data){
              ...
              $('h4.modal-title').html(network+'<span id="inProgress" style="display:none"> Processing... <img src="<?php echo get_absolute_url();?>img/loader-small.gif"></span>');
    ...
    dni = $(this).data('dni-id');
		...
		tablesorterPagerOptions.ajaxUrl = '<?php echo get_absolute_url();?>tracking_support/ajax/dni_get_offers.php?all_offers&dni='+dni+'&offset={page}&limit={size}&{sortList:column}&{filterList:filter}';
  ...
  $('#dniSearchOffersModal').on('hidden.bs.modal', function (e) {
    ...
    $('h4.modal-title').html('<span id="inProgress" style="display:none"> Processing... <img src="<?php echo get_absolute_url();?>img/loader-small.gif"></span>');
...
$(document).on('click', '.toggle', function(e) {
  ...
  if (child.length > 0) {
    ...
    loading.show();
    $.get('<?php echo get_absolute_url();?>tracking_support/ajax/dni_get_offers.php?get_offer&dni='+dni+'&offer_id='+offer_id+'', function(data) {
      //console.log(data);
      try {
...
$(document).on('click', 'button.requestOffer', function(e) {
  ...
  $.get('<?php echo get_absolute_url();?>tracking_support/ajax/dni_get_offers.php?request_offer_access&dni='+dni+'&offer_id='+offer_id+'&type='+type+'', function(data) {
    //console.log(data);
    try {
      // If there is error in back-end, do nothing
      JSON.parse(data);
...
$(document).on('submit', 'form[name="offersQuestionsForm"]', function(e) {
  ...
  btn.button('loading');
  $.ajax({
    url: '<?php echo get_absolute_url();?>tracking_support/ajax/dni_get_offers.php?submit_offer_questions&dni='+dni+'&offer_id='+offer_id+'',
    type: 'POST',
    data: formData
...
$(document).on('click', 'button.setupOffer', function(e) {
  ...
  $.get('<?php echo get_absolute_url();?>tracking_support/ajax/dni_get_offers.php?setup_offer&ddlci=<?php echo $ddlci;?>&dni='+dni+'&offer_id='+offer_id, function(data) {
