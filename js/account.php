...
$(document).ready(function() {
	...
	$('input[name=maxmind-isp]').on("change.radiocheck", function(){
        var checkbox = $(this);
        $.post("<?php echo get_absolute_url();?>account/administration.php", { maxmind: checkbox.val()})
		  .done(function(data) {
		  	if(data){
    ...
    $('#generate-new-api-key').click(function(){
    	...
    	$.post("<?php echo get_absolute_url();?>account/account.php", { add_rest_api_key: true, rest_api_key: key})
		  .done(function(data) {
		  	$("#no-api-keys").remove();
...
$(document).on('click', '#delete-rest-key', function() {
    ...
    $.post("<?php echo get_absolute_url();?>account/account.php", { remove_rest_api_key: true, rest_api_key: key})
		.done(function(data) {
